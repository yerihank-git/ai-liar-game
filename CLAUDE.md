...
- **커밋 메시지**: 한국어, 기능 단위로 커밋. **무엇을(what)이 아닌 왜(why)를 중심으로** 작성
...
# 좋은 예 — 왜 했는지, 어떤 결정을 내렸는지 기술
feat: AI 설명에 2~4초 랜덤 딜레이 추가

즉각 응답 시 AI임이 티가 나서 게임 몰입감을 해침.
실제 플레이어가 타이핑하는 것처럼 느껴지도록 랜덤 딜레이 삽입.
```

```text
fix: Realtime 구독 해제 누락으로 인한 메모리 누수 수정

페이지 이탈 시 cleanup이 없어 Supabase 채널이 계속 열려있었음.
useEffect return으로 unsubscribe 처리하여 해결.
```
